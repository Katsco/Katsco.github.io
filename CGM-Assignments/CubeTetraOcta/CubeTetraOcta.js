...
//var whShape = 0; // Removed b/c instructions
...
function render() {
    ...
    //gl.drawArrays(gl.TRIANGLES, 0, points.length); // Removed

    transMat = transMatCube;
    gl.uniformMatrix4fv(transMatLoc, false, flatten(transMat));
    scaleMat = scalem(cubeScale / 4, cubeScale / 4, cubeScale / 4); // Added
    gl.uniformMatrix4fv(scaleMatLoc, false, flatten(scaleMat)); // Added
    if (cSpinning)
        cTheta[axis] += 2.0;
    gl.uniform3fv(thetaLoc, cTheta); // Added
    // Render cube
    gl.drawArrays(gl.TRIANGLES, 0, totCubePts);

    transMat = transMatTetra;
    gl.uniformMatrix4fv(transMatLoc, false, flatten(transMat));
    scaleMat = scalem(tetraScale / 4, tetraScale / 4, tetraScale / 4); // Added
    gl.uniformMatrix4fv(scaleMatLoc, false, flatten(scaleMat)); // Added
    if (tSpinning)
        tTheta[axis] += 2.0;
    gl.uniform3fv(thetaLoc, tTheta); // Added
    // Render tetrahedron
    gl.drawArrays(gl.TRIANGLES, totCubePts, totTetraPts);

    transMat = transMatOcta;
    gl.uniformMatrix4fv(transMatLoc, false, flatten(transMat));
    scaleMat = scalem(octaScale / 4, octaScale / 4, octaScale / 4); // Added
    gl.uniformMatrix4fv(scaleMatLoc, false, flatten(scaleMat)); // Added
    if (oSpinning)
        oTheta[axis] += 2.0;
    gl.uniform3fv(thetaLoc, oTheta); // Added
    // Render octahedron
